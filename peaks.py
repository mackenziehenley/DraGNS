...
def merge_overlapping_peaks(peaks_df):
    """
    Merge peaks whose FWHM regions overlap, keeping only the most intense peak
    from each overlapping group.
    
    Parameters
    ----------
    peaks_df : pandas.DataFrame
        DataFrame with peak information including left_freq, right_freq, intensity
        
    Returns
    -------
    pandas.DataFrame
        Filtered DataFrame with overlapping peaks merged
    """
    if len(peaks_df) <= 1:
        return peaks_df
    
    # Sort by frequency to process peaks in order
    df = peaks_df.sort_values("freq_cm^-1").reset_index(drop=True)
    
    merged_peaks = []
    i = 0
    
    while i < len(df):
        current_peak = df.iloc[i]
        overlapping_group = [i]  # Start group with current peak
        
        # Find all peaks that overlap with the current peak
        j = i + 1
        while j < len(df):
            next_peak = df.iloc[j]
            
            # Check if FWHM regions overlap
            # Overlap occurs if: left_freq of next peak < right_freq of current peak
            if next_peak["left_freq"] <= current_peak["right_freq"]:
                overlapping_group.append(j)
                # Update the "current" peak to have the rightmost boundary
                if next_peak["right_freq"] > current_peak["right_freq"]:
                    current_peak = next_peak
                j += 1
            else:
                break
        
        # From the overlapping group, keep only the peak with highest intensity
        group_intensities = df.iloc[overlapping_group]["intensity"]
        max_intensity_idx = overlapping_group[group_intensities.argmax()]
        merged_peaks.append(max_intensity_idx)
        
        # Move to the next non-overlapping peak
        i = overlapping_group[-1] + 1
    
    # Return the merged peaks
    merged_df = df.iloc[merged_peaks].reset_index(drop=True)
    
    if len(merged_df) < len(df):
        print(f"Merged overlapping peaks: {len(df)} → {len(merged_df)} peaks")
...
def analyze_peaks(freq_cm1, intensity_cm1,
    ...
    # Sort by frequency
    df_sorted = df_filtered.sort_values("freq_cm^-1").reset_index(drop=True)
    
    # Merge overlapping peaks (keep the most intense peak when FWHM regions overlap)
    df_merged = merge_overlapping_peaks(df_sorted)
