...
# Generate 5x5x3 supercell
nx, ny, nz = 5, 5, 3
total_molecules = 4 * nx * ny * nz  # 300 molecules
atom_id = 1
mol_id = 1

print("TITLE     Ice Ih 5x5x3 Supercell - Wurtzite Structure")
print("REMARK    300 water molecules with hexagonal rings")
print("REMARK    Box dimensions: 22.59 x 22.59 x 22.07 Angstroms")
print("REMARK    Box dimensions: 2.259 x 2.259 x 2.207 nm")
print(f"CRYST1{a*nx:9.3f}{a*ny:9.3f}{c*nz:9.3f}  90.00  90.00 120.00 P 63/m m c  {total_molecules:4d}")
...
print("END")
print(f"\n# Total molecules: {total_molecules}", file=__import__('sys').stderr)
print(f"# Box size: {a*nx:.3f} x {a*ny:.3f} x {c*nz:.3f} Angstroms", file=__import__('sys').stderr)
print(f"# Box size: {a*nx/10:.3f} x {a*ny/10:.3f} x {c*nz/10:.3f} nm", file=__import__('sys').stderr)
